...
module.exports = {
    ...
    methods: {
        fetch: function (successHandler) {
            var that = this
            client({path: this.config.apiUrl}).then(
                function (response) {
        ...
        createNewFromHTML (el) {
            ...
            if (newRow) {
                client({path: this.config.apiUrl, entity: newRow}).then(
                    function (response, status) {
        ...
        updateTable() {
            ...
            this.changedRows.forEach(function (index) {
                ...
                client({path: this.config.apiUrl + "/" + row.id, entity: row, method: 'PUT'}).then(
                    function (response, status) {
        ...
        deleteDog: function (index) {
            var that = this
            client({path: this.config.apiUrl +'/' + this.dogs[index].id, method: 'DELETE'}).then(
                function (response) {
